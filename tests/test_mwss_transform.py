...
class LogicTests(unittest.TestCase):
    ...
    def test_aggregate_fortnightly_changes(self):
        ...
        for qid in ("90f", "91f", "92f", "93f", "94f", "95f", "96f", "97f"):
            with self.subTest(qid=qid):
                ...
                result = fn("90", {qid: "No"}, True)
                self.assertFalse(result)
                result = fn("90", {qid: "Yes"}, False)
    ...
    def test_radio_button_logic(self):
        ...
        for qid in ("92w", "94w", "92f", "94f"):
            with self.subTest(qid=qid):
                result = fn("90", {qid: ""}, True)
                self.assertFalse(result)
                result = fn("90", {qid: "No significant change"}, True)
                self.assertFalse(result)
        ...
        for qid in ("192m", "194m", "192w4", "194w4", "192w5", "194w5"):
            with self.subTest(qid=qid):
                ...
                self.assertFalse(result)
                result = fn("190", {qid: "No significant change"}, True)
                self.assertFalse(result)
                result = fn("190", {qid: "Any other string"}, False)
                self.assertTrue(result)
    ...
    def test_aggregate_fourweekly_changes(self):
        ...
        for qid in ("190w4", "191w4", "192w4", "193w4", "194w4", "195w4", "196w4", "197w4"):
            with self.subTest(qid=qid):
                result = fn("190", {qid: ""}, True)
                self.assertFalse(result)
                result = fn("190", {qid: "No"}, True)
                self.assertFalse(result)
                result = fn("190", {qid: "Yes"}, False)
                self.assertTrue(result)
    ...
    def test_aggregate_weekly_comments(self):
        ...
        for qid in ("300w", "300f", "300m", "300w4", "300w5"):
            with self.subTest(qid=qid):
                ...
                result = fn("300", {"300": "First comment", qid: "Second comment"}, "")
    ...
    def test_aggregate_monthly_paid_employees(self):
        ...
        for qid in ("140m", "140w4", "140w5"):
            result = fn("140", {"140": "125000", qid: "25000"}, 0)
            self.assertEqual(150000, result)
...
class TransformTests(unittest.TestCase):
    ...
    def test_unsigned_decimals(self):
        ...
        for qNr in digits_ingested_as_bools:
            ...
            with self.subTest(qNr=qNr, qid=qid):
                result = MWSSTransformer.transform({qid: "64.0"})
                self.assertIs(True, result[qid])
                self.assertEqual(1, CSFormatter.pck_value(qid, result[qid]))
    ...
    def test_digits_to_onetwo(self):
        ...
        for qNr in digits_ingested_as_bools:
            qid = str(qNr)
            with self.subTest(qNr=qNr, qid=qid):
                result = MWSSTransformer.transform({qid: "64"})
                self.assertIs(True, result[qid])
                self.assertEqual(1, CSFormatter.pck_value(qid, result[qid]))
                result = MWSSTransformer.transform({qid: ""})
                self.assertEqual(2, CSFormatter.pck_value(qid, result[qid]))
    ...
    def test_pay_frequency_as_bool(self):
        ...
        for q, val in pay_frequencies.items():
            ...
            with self.subTest(qid=qid, val=val):
                result = MWSSTransformer.transform({qid: val})
                self.assertIs(True, result[qid])
                self.assertEqual(1, CSFormatter.pck_value(qid, result[qid]))
                result = MWSSTransformer.transform({qid: ""})
                self.assertIs(False, result[qid])
                self.assertEqual(2, CSFormatter.pck_value(qid, result[qid]))
                result = MWSSTransformer.transform({})
                self.assertIs(False, result[qid])
                self.assertEqual(2, CSFormatter.pck_value(qid, result[qid]))
    ...
    def test_dates_to_onetwo(self):
        ...
        for qNr in dates_ingested_as_bools:
            qid = str(qNr)
            with self.subTest(qNr=qNr, qid=qid):
                result = MWSSTransformer.transform({qid: "23/4/2017"})
                self.assertEqual([datetime.date(2017, 4, 23)], result[qid])
                self.assertEqual(1, CSFormatter.pck_value(qid, result[qid]))
                result = MWSSTransformer.transform({qid: ""})
                self.assertEqual([], result[qid])
                self.assertEqual(2, CSFormatter.pck_value(qid, result[qid]))
    ...
    def test_aggregate_fourweekly_changes(self):
        ...
        for qid in ("190w4", "191w4", "192w4", "193w4", "194w4", "195w4", "196w4", "197w4"):
            with self.subTest(qid=qid):
                ...
                result = MWSSTransformer.transform({qid: "No"})
                self.assertIs(False, result["190"])
                result = MWSSTransformer.transform({qid: "Yes"})
    ...
    def test_aggregate_weekly_comments(self):
        ...
        for qid in ("300w", "300f", "300m", "300w4", "300w5"):
            with self.subTest(qid=qid):
                result = MWSSTransformer.transform({qid: "This is a comment"})
    ...
    def test_aggregate_monthly_paid_employees(self):
        ...
        for qid in ("140m", "140w4", "140w5"):
            with self.subTest(qid=qid):
                result = MWSSTransformer.transform({qid: "25"})
